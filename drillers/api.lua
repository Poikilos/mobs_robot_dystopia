-- retroblocks Api (9th September 2015)
drillers = {}
drillers.mod = "redo"

-- Load settings
local damage_enabled = minetest.setting_getbool("enable_damage")
local peaceful_only = minetest.setting_getbool("only_peaceful_drillers")
local disable_blood = minetest.setting_getbool("drillers_disable_blood")
drillers.protected = tonumber(minetest.setting_get("drillers_spawn_protected")) or 1
drillers.remove = minetest.setting_getbool("remove_far_drillers")

function drillers:register_mob(name, def)
	minetest.register_entity(name, {
		...
		blood_texture = def.blood_texture or "drillers_blood.bmp",
			...
			and self.state ~= "attack" then
				...
				for _, oir in pairs(minetest.get_objects_inside_radius(s,self.view_range)) do
					...
					and obj.type == "monster" then
						-- attack monster
						pos = obj.object:getpos()
						distance = ((p.x - s.x) ^ 2 + (p.y - s.y) ^ 2 + (p.z - s.z) ^ 2 )^ 0.5
...
and not lp then
	...
	return
			...
			elseif self.state == "attack" and self.attack_type == "explode" then 
				...
				if self.attack.dist > 3 then
					...
					if self.timer > 3 then
						...
						entity_physics(pos, 3) -- hurt player/drillers caught in blast area
						...
						self.object:remove()
						pos.y = pos.y - 1
						drillers:explosion(pos, 2, 0, 1, self.sounds.explode)
					end
				end
				-- end of exploding drillers
			...
			and self.attack_type == "dogfight" then
				...
				self.object:setyaw(yaw)
				-- attack distance is 2 + half of mob width so the bigger drillers can attack (like slimes)
		...
		get_staticdata = function(self)
			...
			if drillers.remove and self.remove_ok and not self.tamed then
		...
		on_punch = function(self, hitter, tflp, tool_capabilities, dir)
			...
			-- attack puncher and call other drillers for help
...
drillers.spawning_drillers = {}

function drillers:spawn_specific(name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height)
	drillers.spawning_drillers[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not drillers.spawning_drillers[name] then
			--or not pos then
				return
			end
			...
			-- drillers cannot spawn inside protected areas if enabled
			if drillers.protected == 1
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[drillers] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function drillers:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height)
	drillers:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30, chance, active_object_count, -31000, max_height)
end
...
-- explosion
function drillers:explosion(pos, radius, fire, smoke, sound)
...
function drillers:register_arrow(name, def)
...
-- Spawn Egg
function drillers:register_egg(mob, desc, background, addegg)
	local invimg = background
	if addegg == 1 then
		invimg = invimg.."^drillers_chicken_egg.png"
	end
...
function drillers:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and not self.child then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			if tool:is_empty() then
				chance = chance_hand
			elseif tool:get_name() == "drillers:net" then
				chance = chance_net
				tool:add_wear(4000) -- 17 uses
				clicker:set_wielded_item(tool)
			elseif tool:get_name() == "drillers:magic_lasso" then
				-- pick up if owner
				chance = chance_lasso
...
function drillers:feed_tame(self, clicker, feed_count, breed, tame)
